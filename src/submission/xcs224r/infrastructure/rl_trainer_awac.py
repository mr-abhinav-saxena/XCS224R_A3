...
class RL_Trainer(object):
    ...
    def __init__(self, params: Dict[str, Any]) -> None:
        ...
        # Check if this is a Pointmass environment (case-insensitive)
        is_pointmass = 'pointmass' in self.params['env_name'].lower()
        
        # Determine if we need render_mode for video recording
        needs_video = ('env_wrappers' in self.params) or ('non_atari_colab_env' in self.params and self.params['video_log_freq'] > 0)
        
        if needs_video and not is_pointmass:
        ...
        if 'env_wrappers' in self.params and not is_pointmass:
        ...
        if 'non_atari_colab_env' in self.params and self.params['video_log_freq'] > 0 and not is_pointmass:
            self.env = wrappers.RecordVideo(self.env, os.path.join(self.params['logdir'], "gym"), episode_trigger=self.episode_trigger)
            self.eval_env = wrappers.RecordVideo(self.eval_env, os.path.join(self.params['logdir'], "gym"), episode_trigger=self.episode_trigger)
            self.mean_episode_reward = -float('nan')
            self.best_mean_episode_reward = -float('inf')
        
        # Initialize episode reward tracking for Pointmass environments
        if is_pointmass:
            self.mean_episode_reward = -float('nan')
            self.best_mean_episode_reward = -float('inf')
        
        # Store seed for use in reset() - gymnasium no longer uses env.seed()
        self.seed = seed
